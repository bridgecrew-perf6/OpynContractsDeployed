...
const ControllerProxy = artifacts.require('OwnedUpgradeabilityProxy');
...
module.exports = async function (deployer, network, accounts) {
  ...
  //deploy WETH9
  await deployer.deploy(Weth, { from: deployerAddress })
  const weth = await Weth.deployed()

  //deploy Address Library
  await deployer.deploy(Address, {from : deployerAddress});
  await deployer.link(Address, PayableProxyController);

  //deploy payableProxyController
  await deployer.deploy(PayableProxyController, controller.address, pool.address, weth.address, {
    from: deployerAddress,
  });

  //deploy ControllerProxy
  await deployer.deploy(ControllerProxy, { from: deployerAddress });
}
